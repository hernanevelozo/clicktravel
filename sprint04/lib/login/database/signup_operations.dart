...
class SignupOperations {
  SignupOperations loginOperations;
  ...
  //void createUser(Map<String, String> user) async {
  ...
  void createUser(Map<String, String> user) async {
    //final snapshot = await FirebaseDatabase.instance.ref().ref.child('users').get();
    //List values = snapshot.value;
    //int key = values.length;
    //print(values.length);

    final ref = FirebaseDatabase.instance.ref("users");
    //final newKey = await int.parse(ref.key) + 1;

    //print(newKey);

    //final newref = await ref.push().key;
    
    //await ref.child(newref).set({
    await ref.push().set({
      //"${key}" : {
      "userCPF": user['userCPF'],
      "userEmail": user['userEmail'],
  ...
  //Future<bool> validateUser(Map<String, String> user) async {
    ////var usersList = new List<UserElement>();
    //final ref = FirebaseDatabase.instance.ref();
    //final snapshot = await ref.child('users').get();
    ...
    //if (snapshot.exists) {
      ...
      ////var key = 0;
      //for (var i in values) {
        //UserElement.fromMap(i).show();
        //if (UserElement.fromMap(i).userEmail == user['email']) print('email true');
        //if (UserElement.fromMap(i).userPassword == user['password']) print('password true');
        //print(UserElement.fromMap(i).userPassword);
        //print(user['password']);

        //if (UserElement.fromMap(i).userEmail == user['email'] && 
          //UserElement.fromMap(i).userPassword == user['password']) {
          //print(true);
          //return true;
        //}
        ////print("bo");
        ////placeList.add(PlaceElement.fromMap(i, key));
        ////usersList.add(UserElement.fromMap(i));
        ////key++;
  ...
  //Future<int> getUserId(String userEmail) async {
    ...
    //if (snapshot.exists) {
      ...
      //for (var i in values) {
        //UserElement.fromMap(i).show();
        //if (UserElement.fromMap(i).userEmail == userEmail) print('email true');

        //if (UserElement.fromMap(i).userEmail == userEmail)
          //print(true);
          //return key;
        //}
        ////print("bo");
